---
tip: translate by openai@2023-06-08 07:44:41
...

REP: 137 Title: ROS distribution files Author: Tully Foote, Dirk Thomas, Paul Mathieu Status: Accepted Type: Standards Track Content-Type: text/x-rst Created: 04-Feb-2013 Post-History: 19-April-2013

> 回复：137 标题：ROS发行文件 作者：Tully Foote、Dirk Thomas、Paul Mathieu 状态：已接受 类型：标准跟踪 内容类型：text/x-rst 创建日期：2013年2月4日 发布历史：2013年4月19日
...
> 这个REP指定了一组文件，它们定义了ROS发行版，并促进构建、打包、测试和文档过程。其目的是将现有的基础设施形式化，并简化（可能定制）ROS构建农场的托管。
...
> 一个ROS发行版包含许多由各种贡献者维护的软件包，形成一个连贯的集合。到目前为止，只有一个构建农场负责运行单元测试并为各种Ubuntu版本生成Debian软件包，而且迄今为止还没有给出这个过程中使用的文件的语义的正式描述。扩展当前功能以支持其他平台和架构将是重要的扩展。
...
> 这个REP旨在为各种平台和架构发布和打包ROS发行版指定所有信息和文件格式。通过形式化这些数据结构，应该能够：
...
> 简化构建农场的设置，使其可以构建、测试和文档化ROS发行版，即在单独的构建农场上构建、测试和文档化各个ROS发行版。
...
> 应该有一个单一的配置，其中包含（或引用）运行上述进程所需的所有信息。该配置必须易于人类和机器阅读和编辑，并可扩展未来功能。
...
> 发布过程应该独立于构建过程。测试可以在上游源或发布的tarball上执行（取决于上游源是否包含足够的信息来运行测试套件）。构建过程只能处理已发布的软件包。
...
> 一个包含或引用所有所需信息的单一配置，可以为上述流程中的自动处理提供单一入口。
...
> 按平台和架构指定单独的软件包子集是必要的，因为并不是所有发布的软件包都可以并且应该在所有平台和架构上构建。
...
> 信息应当被分离，以便轻松定制各个过程。这尤其必要，因为不同的过程可能由不同的实体运行。
...
> 这个REP为用于该目的的文件提供了一个正式的规范。目前信息存储在：\* 分发文件（groovy.yaml）\* targets.yaml \* 用于文档的个人rosinstall文件和依赖文件
...
> 对分发文件的更改很小，如重组和添加类型和版本字段。 targets.yaml文件的内容将被拆分并合并到发行、源和文档文件及其相应的构建文件中。
...
> 单个rosinstall文件的内容被合并到一个文件中。文档的依赖项被替换为引用同一文件中指定的其他存储库。
...
> 索引文件充当单个配置入口点，列出所有可用的ROS发行版，并附带指向进一步处理特定信息的引用。这些引用可以是相对于索引文件的，也可以是绝对的。
...
> - release_cache：引用一个发布缓存。实现要求这个字段是一个字典、一个列表或一个标量，具体留给实现者决定。以下示例假定实现需要一个 URL。
...
> 状态：定义存储库的软件包的状态。可以是以下之一：`developed`，`maintained`，`unmaintained`，`end-of-life`。
...
> 一个可选的字段，用简短的句子描述仓库的当前状态。例如，详细说明EOL的原因以及推荐的升级路径。
...
> - 包：可选的包名列表。如果没有指定包，则假定存储库根目录中有一个名为存储库的包。可以使用以下可选键来指定每个包名：
...
> 标签：一个字典标签，可由Bloom和ROSInstall生成器等工具使用，避免这些工具需要猜测发布存储库中的标签，例如“release / package / 1.2.3”与“release / groovy / package / 1.2.3-4”。这为使用发布存储库中的标签的工具提供了有用的未来保护机制，而不是猜测它们的格式，而是可以直接从标签模板中推断它们。
...
> 平台：一个目标平台的字典，发布软件包。每个键包含操作系统名称，而值是操作系统代码名称的列表。这些操作系统名称和操作系统代码名称由*rospkg.os_detect*[^2]确定。每个目标平台将导致不同的bloom发布。
...
> `- package_whitelist：要构建的软件包名称列表。如果省略，则构建发行文件中指定的所有软件包。任何上游软件包隐式包含在内。`
...
> - 包黑名单：不参与构建的包名列表。如果省略，则不排除任何包。任何下游包都会被隐式排除。黑名单会覆盖白名单。
...
> 目标：用于构建软件包的嵌套字典。第一级键包含操作系统名称。第二级键包含操作系统代码名称。第三级键包含CPU架构。指定的操作系统名称和操作系统代码名称必须列为相应发行版或源文件中的平台。每个级别可以包含一个[\_config]{.title-ref}键，其中可以包含一个具有任意数据的字典。这些配置可用于指定特定目标的信息（例如[apt_target_repository]{.title-ref}或[yum_repository]{.title-ref}）。
...
> 指定同步包所需满足的标准。可以设置以下任何一个选项，并且必须满足所有选项：
...
> 所有ROS发行版的元信息的集合，包括所有package.xml文件中的信息。缓存必须引用发行文件并存储发行文件的哈希值，以便能够检测缓存是否无效。该缓存的格式被认为是实现细节，并未在本REP中指定。
...
> 源文件使用与发布文件类似的规范，但没有平台列表或每个存储库的附加元数据（状态、状态描述、包和标签）。由于存储库类型不限于git，因此每个存储库都必须指定一种类型。
...
> - 版本：对于Git和Hg，这是要检查出的标签、分支或哈希值。对于SVN，由于分支/标签已编码在URL中，因此不应设置版本。
- type: must be \'source\'
- version: version number, this REP describes version 1
...
> 源构建文件使用的规范类似于发布构建文件，但没有同步信息。此外，白名单和黑名单是基于仓库级别的，不考虑任何类型的依赖关系。
...
> - repository_blacklist：排除在构建之外的存储库名称列表。如果省略此项，则不排除任何存储库。黑名单优先于白名单。
...
> 文档文件使用与源文件类似的规范。此外，每个存储库都可以指定它所依赖的其他存储库的列表。
...
> - 版本：对于Git和Hg，这是要检查的标签、分支或哈希值。对于SVN，不应设置版本，因为分支/标签已编码到URL中。
...
> - 依赖：存储库名称列表。其他存储库在文档中进行交叉引用。仅在未发布依赖存储库中的包时才需要此步骤（默认值：\[\]）
- type: must be \'doc\'
- version: version number, this REP describes version 1
...
> - 仓库黑名单：要排除在构建之外的仓库名称列表。如果省略此项，则不排除任何仓库。黑名单会覆盖白名单。
...
> 这个REP将被实现在一个叫做*rosdistro*的Python模块中。该模块将作为本REP的参考实现。任何ROS工具请求本REP定义的信息都应该使用这个参考API实现。
...
> 由于新文件存储在不同的URL（不在rosdistro存储库的发布子文件夹中）下，当信息不再从旧URL获取时，旧工具将会失败。
...
> [^6]: 示例文件指定ROS发行版：[https://github.com/ros-infrastructure/rosdistro/tree/master/test/files](https://github.com/ros-infrastructure/rosdistro/tree/master/test/files)
